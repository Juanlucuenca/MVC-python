...
class ContactController:
    ...
    def get_contacts_with_secondary_by_user(self, username):
        contact_repository = ContactRepository()
        contacts = contact_repository.get_contacts_with_secondary_by_user(username)
        contactsForView = []
        for contact in contacts:
            contactForView = ContactForView(contact)
            contactsForView.append(contactForView)
    ...
    def get_contact_by_secondary(self, username, secondary):
        contact_repository = ContactRepository()
        contacts = contact_repository.get_contact_by_secondary(secondary)
        contactsForView = []
        for contact in contacts:
            contactForView = ContactForView(contact)
            contactsForView.append(contactForView)
