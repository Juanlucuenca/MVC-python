from ..Entities.Contact import Contact
from ..Context import Context

class ContactRepository:

    def get_contacts_by_user(self, username):
        with Context() as context1:
            query = "SELECT * FROM contact WHERE state = 1 && username = %s"
            values = (username,)
            context1.mycursor.execute(query,values)
            contactsDB = context1.mycursor.fetchall()

        contacts = []
        for contactDB in contactsDB:
            contact = Contact()
            contact.id = contactDB[0]
            contact.name = contactDB[1]
            contact.surname = contactDB[2]
            contact.email = contactDB[3]
            contact.username = contactDB[4]
            contact.colegioSecundario = contactDB[5]
            contact.state = contactDB[6]
            contacts.append(contact)
        return contacts

    def get_contacts_with_secondary_by_user(self, username):
        with Context() as context1:
            query = "SELECT * FROM contact WHERE colegioSecundario <> 'none' && username = %s"
            values = (username,)
            context1.mycursor.execute(query,values)
            contactsDB = context1.mycursor.fetchall()
        contacts = []
        for contactDB in contactsDB:
            contact = Contact()
            contact.id = contactDB[0]
            contact.name = contactDB[1]
            contact.surname = contactDB[2]
            contact.email = contactDB[3]
            contact.username = contactDB[4]
            contact.colegioSecundario = contactDB[5]
            contact.state = contactDB[6]
            contacts.append(contact)
        return contacts
    
    def get_contact_by_secondary(self, colegioSecundario):
        with Context() as context1:
            query = "SELECT * FROM contact WHERE colegioSecundario = %s and colegioSecundario <> 'none'"
            values = (colegioSecundario,)
            context1.mycursor.execute(query,values)
            contactsDB = context1.mycursor.fetchall()
        contacts = []
        for contactDB in contactsDB:
            contact = Contact()
            contact.id = contactDB[0]
            contact.name = contactDB[1]
            contact.surname = contactDB[2]
            contact.email = contactDB[3]
            contact.username = contactDB[4]
            contact.colegioSecundario = contactDB[5]
            contact.state = contactDB[6]
            contacts.append(contact)
        return contacts
    
    def get_contact(self, contact):
        with Context() as context1:
            query = "SELECT * FROM contact WHERE idcontact = %s"
            values = (contact.id,)
            context1.mycursor.execute(query, values)
            contactDB = context1.mycursor.fetchone()
            contact = Contact()
            contact.id = contactDB[0]
            contact.name = contactDB[1]
            contact.surname = contactDB[2]
            contact.email = contactDB[3]
            contact.username = contactDB[4]
            contact.state = contactDB[5]
        return contact
    
    def add_contact(self, contact):
        with Context() as context1:
            if contact.colegioSecundario == "":
                contact.colegioSecundario = 'none'
            query = "INSERT INTO contact (name, surname, email, username, colegioSecundario, state) VALUES (%s, %s, %s, %s, %s, %s)"
            values = (contact.name, contact.surname, contact.email, contact.username, contact.colegioSecundario , contact.state)
            context1.mycursor.execute(query, values)
            context1.mydb.commit()
            contact.id = context1.mycursor.lastrowid
        return contact

    def update_contact(self, contact):
        with Context() as context1:
            query = "UPDATE contact SET name = %s, surname = %s, email = %s WHERE idcontact = %s AND username = %s"
            values = (contact.name, contact.surname, contact.email, contact.id, contact.username)
            context1.mycursor.execute(query, values)
            context1.mydb.commit()

    def delete_contact(self, contact):
        with Context() as context1:
            query = "UPDATE contact SET state = %s WHERE idcontact = %s AND username = %s"
            values = (0, contact.id, contact.username)
            context1.mycursor.execute(query, values)
            context1.mydb.commit()
        
